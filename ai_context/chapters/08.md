...
erDiagram
    ...
    contract_encounters {
        string contract_encounter_id PK
        string contract_id FK
        string encounter_id FK
        boolean fulfills_objective
        datetime reported_on
        boolean proof_validated
    }
    ...
    encounter_targets {
        ...
        enum target_role
        number quantity
        string involvement_notes
    }
    
    encounter_reports {
        string report_id PK
        string encounter_id FK
        string participants_summary
        string loot_summary
    ...
    contracts ||--o{ contract_encounters : "logs"
    monster_hunting_contracts ||--o{ contract_encounters : "fulfilled_by"
    encounters ||--o{ contract_encounters : "linked_via"
    encounters ||--o{ encounter_targets : "features"
    encounters ||--o{ encounter_reports : "documented_by"
...
// EncounterType, EncounterStatus, EncounterParticipantRole, and EncounterTargetRole are defined in Chapter IV's combat schema and reused here.

type ContractEncounterRecord = {
  contractEncounterId: string;
  contractId: string;
  encounterId: string;
  fulfillsObjective: boolean;
  reportedOn: string;
...
type ContractTargetLink = {
  targetLinkId: string;
  encounterId: string;
  monsterId: string;
  targetRole: EncounterTargetRole;
  quantity: number;
...
� Every hunt logs its field action into the shared encounters ledger (see Chapter IV) for combat analytics and status tracking
� Contract_encounters links contract paperwork to the recorded encounter outcome for proof validation and objective tracking
� Encounter_reports replace the legacy monster_encounters log by storing participants, loot summaries, and narrative details
